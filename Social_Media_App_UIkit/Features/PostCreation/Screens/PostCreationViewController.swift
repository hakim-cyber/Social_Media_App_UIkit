...
class PostCreationViewController: UIViewController {
    ...
    @objc private func postTapped() {
        Task{
          await self.vm.createPost(caption: captionTextField.textView.text)
        }
    ...
    func bindViewModel(){
        ...
        vm.$selectedLocation
            ...
            .sink { [weak self] location in
                self?.locationButton.locationText = location
            }
            .store(in: &cancellables)
    ...
    func setLocationButton(){
        ...
        locationButton.onTap = { [weak self] in
            self?.vm.tappedLocation()
        }
        locationButton.onClear = { [weak self] in
            self?.vm.clearLocation()
        }
